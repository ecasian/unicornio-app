...
function FlowHeader({ step }: { step: number }) {
  const steps = ['Repartidor', 'Cliente', 'Surtido'];
  return (
    <header className="bg-fuchsia-950 px-4 pb-7 pt-7 text-white">
      <div className="mx-auto max-w-3xl">
        ...
        <ol aria-label="Progreso" className="mt-6 grid grid-cols-3 gap-2">
...
function SurtidoResumen({ stock }: { stock: StockObjetivo[] }) {
  return (
    <>
      ...
      <button type="button" disabled aria-describedby="levantamiento-pendiente"
        className="mt-6 min-h-12 w-full rounded-full bg-fuchsia-700 px-6 font-bold text-white opacity-50">
        Continuar al levantamiento
      </button>
      <p id="levantamiento-pendiente" className="mt-2 text-center text-sm text-slate-600">
        El registro de existencias estará disponible en una etapa posterior.
      </p>
    </>
  );
}
...
export function RepartidorInicioPage() {
  ...
  const paso = !repartidor ? 1 : !cliente ? 2 : 3;
      ...
      <div className="mx-auto max-w-3xl px-4 py-7 sm:px-6">
        {!repartidor ? (
          <section aria-labelledby="seleccionar-repartidor" className="space-y-4">
            ...
            {repartidores.isPending ? <p role="status">Cargando repartidores…</p>
              ...
              : <ul className="space-y-3">{repartidores.data.filter((item) => item.activo).map((item) => <li key={item.id}>
                <button type="button" aria-label={`Seleccionar repartidor ${item.nombre}`} onClick={() => { setRepartidorId(item.id); setClienteId(null); }}
          ...
          <section aria-labelledby="seleccionar-cliente" className="space-y-4">
            <button type="button" onClick={() => { setRepartidorId(null); setClienteId(null); }} className="min-h-11 font-semibold text-fuchsia-800 underline underline-offset-4">← Cambiar repartidor</button>
            ...
            {clientes.isPending ? <p role="status">Cargando clientes…</p>
              ...
              : <ul className="space-y-3">{clientes.data.filter((item) => item.activo).map((item) => <li key={item.id}>
                <button type="button" aria-label={`Seleccionar cliente ${item.nombre}. Dirección: ${item.direccion}`} onClick={() => setClienteId(item.id)}
          ...
          <section aria-labelledby="resumen-surtido" className="space-y-4">
            <div className="flex flex-wrap gap-x-5 gap-y-1 text-sm font-semibold text-fuchsia-800">
              <button type="button" onClick={() => { setRepartidorId(null); setClienteId(null); }} className="min-h-11 underline underline-offset-4">← Cambiar repartidor</button>
              <button type="button" onClick={() => setClienteId(null)} className="min-h-11 underline underline-offset-4">← Cambiar cliente</button>
            </div>
            <p className="text-sm text-slate-600">Repartidor: <strong>{repartidor.nombre}</strong></p>
            ...
            {stock.isPending ? <p role="status">Cargando surtido…</p>
